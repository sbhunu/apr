/**
 * Certificate Verification Page (by number)
 * Handles certificate verification by number from query parameter
 */
...
export default function CertificateVerificationByNumberPage() {
  const searchParams = useSearchParams()
  const router = useRouter()
  const certificateNumberParam = searchParams.get('number')
  ...
  useEffect(() => {
    ...
    if (certificateNumberParam) {
      handleVerify({ certificateNumber: certificateNumberParam })
    }
  ...
  async function handleVerify(request: {
    ...
    try {
      ...
      // If titleId found, redirect to detailed page
      if (data.result && request.certificateNumber && !request.titleId) {
        // Keep on same page for now - could redirect if needed
      }
    } catch (err) {
